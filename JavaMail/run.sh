...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/javamail.sql
}
rabbit_start_stop() {
...
generate_load_url() {
  ...
  read counter
  echo "Please wait generating load for Struts application......."
  for (( i=1; i <=$counter; i++ ))
  do
   curl http://localhost:8080/JavaMailClient/client
   sleep 5
  done
  echo "Done with load generation for Struts application........."
}
