...
generate_load_url() {
  ...
  read counter
  echo "Please wait generating load for MongoDB application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl http://localhost:8080/MongoDBClient/client/
  sleep 5
  done
  echo "Done with load generation for MongoDB application........."
}
