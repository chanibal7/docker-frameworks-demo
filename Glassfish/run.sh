...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/glassfish.sql
}

#Restart Glassfish Server.
glassfish_start_stop() {
  cd /opt/glassfish4/bin
  cp /mnt/tempdircopy/GlassFishClient.war /opt/glassfish4/glassfish/domains/domain1/autodeploy/GlassFishClient.war
  nohup ./asadmin restart-domain domain1 &
  sleep 5
}
...
generate_load_url() {
  ...
  read counter
  echo "Please wait generating load for Glassfish application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl http://localhost:8080/GlassFishClient/client/
  sleep 5
  done
  echo "Done with load generation for Glassfish application........."
}
