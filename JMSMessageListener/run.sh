...
#Restart tomcat
tomcat_start_stop() {
JBPIDS=$(pgrep -d" " -f "JMSMessageListener-Server-Node")
kill -9 $JBPIDS
...
generate_load_url() {
  ...
  read counter
  echo "Please wait generating load for JMSMessageListener application......."
  for (( i=1; i <=$counter; i++ ))
  do
   curl http://localhost:8081/JMSMessageListenerClient/client
   sleep 5
  done
  echo "Done with load generation for JMSMessageListener application........."
