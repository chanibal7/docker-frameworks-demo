...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/lucene.sql
}
...
#generate load for ApacheLucene apps
generate_load_url() {
  sleep 15
  echo "Input number of times URL to be hit"
  read counter
  echo "Please wait generating load for ApacheLucene application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl http://localhost:8080/HelpingServlet/AssociateServlet
  sleep 5
  done
  echo "Done with load generation for ApacheLucene application........."
}
