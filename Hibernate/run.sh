...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/hibernate.sql
}
...
generate_load_url() {
  ...
  read counter
  echo "Please wait generating load for Hibernate application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl http://localhost:8080/hibernateWebserviceClient/client/
  sleep 5
  done
