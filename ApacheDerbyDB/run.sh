...
java_setup() {
  ...
  echo "Class path is set to:": $CLASSPATH
}
...
#generate load for DerbyDB apps
generate_load_url() {
  sleep 15
  echo "Input number of times URL to be hit"
  read counter
  echo "Please wait generating load for DerbyDB application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl --fail --silent --show-error http://localhost:8080/DerbyClient/client/ > /dev/null
  sleep 5
  done
  echo "Done with load generation for DerbyDB application........."
}
