...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/dwr.sql
}
rabbit_start_stop() {
...
cp $CATALINA_SETUP/context.xml $TOMCAT_HOME_TWO/conf/context.xml
...
generate_load_url() {
  ...
  read counter
  echo "Please wait generating load for Dwr application......."
  for (( i=1; i <=$counter; i++ ))
  do
   curl http://localhost:8080/dwrClient/client
   sleep 5
  done
  echo "Done with load generation for Dwr application........."
}
