...
java_setup() {
  ...
  TOMCAT_HOME_TWO=/mnt/dockerdemo/server/jbosstwo
...
machine_agent_download() {
 ...
 rm $MACHINE_AGENT_DIR/conf/app-agent-config.xml
   wget https://s3.amazonaws.com/appdynamics-docker-artifacts/Config-xml-files/jboss/app-agent-config.xml -O  $MACHINE_AGENT_DIR/conf/app-agent-config.xml
...
then
  ...
  rm $AGENT_DIR/conf/app-agent-config.xml
  wget https://s3.amazonaws.com/appdynamics-docker-artifacts/Config-xml-files/jboss/app-agent-config.xml -O  $AGENT_DIR/conf/app-agent-config.xml
...
catalina_setup() {
cp $CATALINA_SETUP/catalina1.sh $TOMCAT_HOME_ONE/bin/catalina.sh
...
#Restart tomcat
tomcat_start_stop() {
echo "Killing running jboss..."
ps -ef | grep jbossone | grep -v grep | awk '{print $2}' | xargs kill -9
echo "Starting jboss..."
nohup $TOMCAT_HOME_TWO/bin/standalone.sh &
pid=$(ps -ef | awk '/jbossone/ {print $2}')
pid=$(tomcat_pid)
...
generate_load_url() {
  ...
  read counter
  echo "Please wait generating load for JbossHornetQ application......."
  for (( i=1; i <=$counter; i++ ))
  do
   curl http://localhost:8080/JbossHornetQClient/client
   sleep 5
  done
  echo "Done with load generation for JbossHornetQ application........."
}
