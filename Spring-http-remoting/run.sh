...
generate_load_url() {
  ...
  read counter
  echo "Please wait generating load for SpringHttpRemoting application......."
  for (( i=1; i <=$counter; i++ ))
  do
   curl http://localhost:8080/SpringHttpClient/httpRemoteCall
   sleep 5
  done
  echo "Done with load generation for SpringHttpRemoting application........."
}
