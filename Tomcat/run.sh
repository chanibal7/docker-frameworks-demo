...
#generate load for Tomcat apps
generate_load_url() {
  sleep 15
  echo "Input number of times URL to be hit"
  read counter
  echo "Please wait generating load for Tomcat application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl http://localhost:8080/TomcatClient/client/
  sleep 5
  done
  echo "Done with load generation for Tomcat application........."
}
