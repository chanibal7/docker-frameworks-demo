...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/servlet.sql
}
...
#generate load for Servlet apps
generate_load_data() {
  sleep 15
  echo "Input number of times URL to be hit"
  read counter
  echo "Please wait generating load for Servlet application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl -d "emailId=xyz%40gmail.com&subject=Apple+WebObjects+Framework&content=dsaf&button1=+Send+" http://localhost:8080/BasicSample/inputSubmit.mailservlet
  sleep 5
  done
  echo "Done with load generation for Servlet application........."
}
