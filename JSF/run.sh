...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/jsf.sql
}
...
generate_load_url() {
  ...
  read counter
  echo "Please wait generating load for JSF application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl http://localhost:8080/JSFClient/client
  sleep 5
  done
  echo "Done with load generation for JSF application........."
}
