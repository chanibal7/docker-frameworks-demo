...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/liferay.sql
}
...
generate_load_url() {
  ...
  read counter
  echo "Please wait generating load for LiferayClient application......."
  for (( i=1; i <=$counter; i++ ))
  do
   curl http://localhost:8080/LiferayClient/client
   sleep 5
  done
  echo "Done with load generation for LiferayClient application........."
}
