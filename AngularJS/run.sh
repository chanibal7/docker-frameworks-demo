...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/angularjs.sql
}
...
#generate load for AngularJS apps
generate_load_url() {
  sleep 15
  echo "Input number of times URL to be hit"
  read counter
  echo "Please wait generating load for AngularJS application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl http://localhost:8080/AngularSpringApp/cars/addCar/honda
  sleep 5
  done
  echo "Done with load generation for AngularJS application........."
}
