...
#Setup java home
java_setup() {
  export AXIS2_HOME=/mnt/axis2/axis2-1.6.2/axis2-1.6.2
...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/equinox.sql
}
...
#generate load for Eclipse Equinox apps
generate_load_url() {
  sleep 20
  cp /mnt/tempdircopy/sample.http_1.0.0.jar  /mnt/dockerdemo/server/tomcattwo/webapps/bridge/WEB-INF/eclipse/plugins/sample.http_1.0.0.jar
  cp /mnt/tempdircopy/sample.http_1.0.0.jar  /mnt/dockerdemo/server/tomcattwo/work/Catalina/localhost/bridge/eclipse/plugins/sample.http_1.0.0.jar
  sleep 15
  echo "Input number of times URL to be hit"
  read counter
  echo "Please wait generating load for Eclipse Equinox application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl http://localhost:8080/HelpingServlet/AssociateServlet
  sleep 5
  done
  echo "Done with load generation for Eclipse Equinox application........."
}
