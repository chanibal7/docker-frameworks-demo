...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/springrabbitmq.sql
}
rabbit_start_stop() {
...
generate_load_url() {
  ...
  read counter
  echo "Please wait generating load for SpringRabbitMQ application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl http://localhost:8080/RabbitMQSpringClient/client
  sleep 5
  done
  echo "Done with load generation for SpringRabbitMQ application........."
}
