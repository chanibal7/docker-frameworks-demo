...
java_setup() {
  ...
  JBOSS_HOME_TWO=/mnt/dockerdemo/server/jbosstwo
...
cd $JBOSS_HOME_TWO/bin/
sed -i "s/HOSTNAME/$INPUT_HOST_NAME/g" run.sh
sed -i "s/PORTNO/$INPUT_PORT/g" run.sh
}
...
#Restart tomcat
tomcat_start_stop() {
JBPIDS=$(pgrep -d" " -f "JbossTree-Server-Node")
if [ -n "$JBPIDS" ]
  then
    echo "JBOSS is already running (JBPIDS: $JBPIDS)"
	kill -9 $JBPIDS
	nohup $JBOSS_HOME_TWO/bin/run.sh &
 else	
	nohup $JBOSS_HOME_TWO/bin/run.sh &
 fi
  pid=$(tomcat_pid)
...
generate_load_url() {
  ...
  read counter
  echo "Please wait generating load for JbossTree application......."
  for (( i=1; i <=$counter; i++ ))
  do
   curl http://localhost:8081/JbossTreeClient/client
   sleep 5
  done
  echo "Done with load generation for JbossTree application........."
  nohup $JBOSS_HOME_TWO/bin/shutdown.sh &
}
