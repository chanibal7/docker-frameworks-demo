...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/cxf.sql
}
...
#generate load for ApacheCXF apps
generate_load_url() {
  sleep 15
  echo "Input number of times URL to be hit"
  read counter
  echo "Please wait generating load for ApacheCXF application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl --fail --silent --show-error http://localhost:8080/spring-cxf-rest/services/productServices/product/1 > /dev/null
  sleep 5
  done
  echo "Done with load generation for ApacheCXF application........."
}
