...
#generate load for PostgresSQL apps
generate_load_url() {
  sleep 15
  echo "Input number of times URL to be hit"
  read counter
  echo "Please wait generating load for PostgresSQL application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl http://localhost:8080/PostgresSQLClient/client/
  sleep 5
  done
  echo "Done with load generation for PostgresSQL application........."
}
