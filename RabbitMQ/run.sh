...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/rabbit.sql
}
rabbit_start_stop() {
...
generate_load_url() {
  ...
  read counter
  echo "Please wait generating load for RabbitMQ application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl http://localhost:8080/RabbitMQClient/client
  sleep 5
  done
  echo "Done with load generation for RabbitMQ application........."
}
