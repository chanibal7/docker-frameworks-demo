...
then
  ...
  unzip agent.zip -d /mnt/dockerdemo/javaagentTwo
  cp /mnt/tempdircopy/controller-info1.xml $AGENT_DIR_TWO/conf/controller-info.xml
...
#generate load for OC4J apps
generate_load_url() {
  sleep 15
  echo "Input number of times URL to be hit"
  read counter
  echo "Please wait generating load for OC4J application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl http://localhost:8888/MySQLClient/client/
  sleep 5
  done
  echo "Done with load generation for OC4J application........."
}
