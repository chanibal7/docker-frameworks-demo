...
java_setup() {
  ...
  export RESIN_HOME=/mnt/resin-pro-4.0.41
...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/resin.sql
}

resins_start_stop() {
echo "Starting Resin Server"
java -jar /mnt/resin-pro-4.0.41/lib/resin.jar stop
java -jar /mnt/resin-pro-4.0.41/lib/resin.jar start
echo "Resin sever is up"
sleep 2
}
...
generate_load_url() {
  ...
  read counter
  echo "Please wait generating load for resin application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl --fail --silent --show-error http://localhost:8081/HelpingServlet/AssociateServlet > /dev/null
  sleep 5
  done
  echo "Done with load generation for resin application........."
}
