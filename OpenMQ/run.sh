...
#Start OpenMQ Server.
openmq_start() {
cd /mnt/server/MessageQueue4_5/mq/bin/
nohup ./imqbrokerd &
}
...
#generate load for OpenMQ apps
generate_load_url() {
  sleep 15
  echo "Input number of times URL to be hit"
  read counter
  echo "Please wait generating load for OpenMQ application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl http://localhost:8080/OpenMQClient/client/
  sleep 5
  done
  echo "Done with load generation for OpenMQ application........."
}
