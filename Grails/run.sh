...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/grails.sql
}
...
generate_load_url() {
  ...
  read counter
  echo "Please wait generating load for Grails application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl http://localhost:8080/GrailsWebserviceClient/client/
  sleep 5
  done
