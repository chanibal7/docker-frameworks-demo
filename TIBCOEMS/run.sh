...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/tibcoems.sql
}
...
# Install TIBCO EMS Server
 cd /mnt/tempdircopy
 ./TIBCOUniversalInstaller-lnx-x86.bin -silent -V TIBCOUniversalInstaller-ems.silent
 cp -rf /mnt/tempdircopy/datastore/ /home/user/tibco/tibco/cfgmgmt/ems/data/
 chmod 777 /home/user/tibco/tibco/cfgmgmt/ems/data/datastore/*
 nohup /opt/tibco/ems/8.1/bin/tibemsd -config /home/user/tibco/tibco/cfgmgmt/ems/data/tibemsd.conf &
...
#generate load for TIBCO ems apps
generate_load_url() {
  sleep 15
  echo "Input number of times URL to be hit"
  read counter
  echo "Please wait generating load for TIBCO ems application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl http://localhost:8080/TibcoEmsClient/client
  sleep 5
  done
  echo "Done with load generation for TIBCO ems application........."
}
