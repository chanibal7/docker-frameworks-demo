...
java_setup() {
  ...
  TOMCAT_HOME_ONE=/mnt/dockerdemo/server/jbossone
...
machine_agent_download() {
 ...
 if [[ $prompt == "y" || $prompt == "Y" || $prompt == "yes" || $prompt == "Yes" ]]
  then
   ...
   wget https://s3.amazonaws.com/appdynamics-docker-artifacts/Config-xml-files/jboss/app-agent-config.xml -O  /mnt/dockerdemo/javaagent/conf/app-agent-config.xml
...
catalina_setup() {
cp $CATALINA_SETUP/catalina2.sh $TOMCAT_HOME_TWO/bin/catalina.sh
...
echo "Starting jboss..."
nohup $TOMCAT_HOME_ONE/bin/standalone.sh &
pid=$(ps -ef | awk '/jbossone/ {print $2}')
echo "Jboss PID After RESTART (pid: $pid)"
pid=$(tomcat_pid)
  if [ -n "$pid" ]
  then
...
#generate load for jboss apps
generate_load_url() {
  sleep 15
  echo "Input number of times URL to be hit"
  read counter
  echo "Please wait generating load for Jboss application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl http://localhost:8080/JbossClient/client
  sleep 5
  done
  echo "Done with load generation for Jboss application........."
}
