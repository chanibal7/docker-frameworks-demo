...
java_setup() {
  ...
  TOMCAT_HOME_ONE=/mnt/dockerdemo/server/tomcateeone
  TOMCAT_HOME_TWO=/mnt/dockerdemo/server/tomcateetwo
  SHUTDOWN_WAIT=20
...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/tomcatee.sql
}
...
#generate load for TomcatEE apps
generate_load_url() {
  sleep 15
  echo "Input number of times URL to be hit"
  read counter
  echo "Please wait generating load for TomcatEE application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl --fail --silent --show-error http://localhost:8080/AngularSpringApp/cars/addCar/honda > /dev/null
  sleep 5
  done
  echo "Done with load generation for TomcatEE application........."
}
