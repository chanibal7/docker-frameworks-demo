...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/jboss.sql
}
#Configure Catalina setup
catalina_setup() {
dos2unix /mnt/tempdircopy/catalina1.sh
dos2unix /mnt/tempdircopy/server2.xml
dos2unix /mnt/tempdircopy/server.xml
cp /mnt/tempdircopy/catalina1.sh /mnt/dockerdemo/server/tomcatone/bin/catalina.sh
cp /mnt/tempdircopy/server2.xml /mnt/dockerdemo/server/tomcatone/conf/server.xml
cd $TOMCAT_HOME_ONE/bin/
sed -i "s/HOSTNAME/$INPUT_HOST_NAME/g" catalina.sh
sed -i "s/PORTNO/$INPUT_PORT/g" catalina.sh
cp /mnt/tempdircopy/server.xml $WEBSPHERE_HOME/profiles/appsvr01/config/cells/bff845416b26Node01Cell/nodes/bff845416b26Node01/servers/server1/server.xml
cd $WEBSPHERE_HOME/profiles/appsvr01/config/cells/bff845416b26Node01Cell/nodes/bff845416b26Node01/servers/server1
sed -i "s/HOSTNAME/$INPUT_HOST_NAME/g" server.xml
sed -i "s/PORTNO/$INPUT_PORT/g" server.xml
}
...
#Restart tomcat
tomcat_start_stop() {
 ps xu | grep server1 | grep -v grep | awk '{ print $2 }' | xargs kill -9
...
#generate load for glassfish apps
generate_load_url() {
  sleep 15
  echo "Input number of times URL to be hit"
  read counter
  echo "Please wait generating load for Websphere application......."
  for (( i=1; i <=$counter; i++ ))
  do
   curl http://localhost:8081/WebsphereClient/client
   sleep 5
  done
  echo "Done with load generation for Websphere application........."  
}
