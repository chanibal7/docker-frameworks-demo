...
#Restart tomcat
tomcat_start_stop() {
JBPIDS=$(pgrep -d" " -f "JbossRMI-Server-Node")
kill -9 $JBPIDS
...
generate_load_url() {
  ...
  read counter
  echo "Please wait generating load for JbossRMI application......."
  for (( i=1; i <=$counter; i++ ))
  do
   curl http://localhost:8081/JbossRMIClient/client
   sleep 5
  done
  echo "Done with load generation for JbossRMI application........."
}
