...
#generate load for Axis2 apps
generate_load_url() {
  cp /mnt/tempdircopy/FirstWebService.aar /mnt/dockerdemo/server/tomcatone/webapps/axis2/WEB-INF/services/FirstWebService.aar
  sleep 15
  echo "Input number of times URL to be hit"
  read counter
  echo "Please wait generating load for Apache Axis2 application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl http://localhost:8081/HelpingServlet/AssociateServlet
  sleep 5
  done
  echo "Done with load generation for Apache Axis2 application........."
}
