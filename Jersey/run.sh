...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/jersey.sql
}
...
#generate load for Jersey apps
generate_load_url() {
  sleep 15
  echo "Input number of times URL to be hit"
  read counter
  echo "Please wait generating load for Jersey application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl -d "name=xyz234&rating=3.6"  http://localhost:8080/BankingApplication/add/process
  sleep 5
  done
  echo "Done with load generation for Jersey application........."
}
