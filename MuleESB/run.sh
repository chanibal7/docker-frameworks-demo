...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/muleesb.sql
}

mule_esb_start() {
cp /mnt/tempdircopy/wrapper.conf /mnt/dockerdemo/server/muleesbserver/mule-standalone-3.5.0/conf/wrapper.conf
cd /mnt/dockerdemo/server/muleesbserver/mule-standalone-3.5.0/bin/
nohup ./mule &
}
...
#generate load for MuleESB apps
generate_load_url() {
  sleep 15
  echo "Input number of times URL to be hit"
  read counter
  echo "Please wait generating load for MuleESB application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl http://localhost:8888/?name=Ross
  sleep 5
  done
  echo "Done with load generation for MuleESB application........."
}
