...
mysql_start_stop() {
  ...
  mysql -uroot -proot "" < /mnt/tempdircopy/xfire.sql
}
...
#generate load for Xfire apps
generate_load_url() {
  sleep 15
  echo "Input number of times URL to be hit"
  read counter
  echo "Please wait generating load for Xfire application......."
  for (( i=1; i <=$counter; i++ ))
  do
  curl --fail --silent --show-error http://localhost:8081/HelpingServlet/AssociateServlet > /dev/null
  sleep 5
  done
  echo "Done with load generation for Xfire application........."
}
